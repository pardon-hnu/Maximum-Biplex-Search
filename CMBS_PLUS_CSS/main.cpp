...
int main(int argc, char** argv) {
    ...
    vector<xypair> XYPairs=core.GetPairs(theta_l,theta_r);
    cout<<"core decomposition"<<endl;
    int maximum=theta_l*(theta_r-k);
    subgraph mbp;    
    vector<xypair>::iterator findmax;
    int xmax=0;
    int ymax=0;
    ...
    while(true)
    {
        ...
        bkmb.edges=theta_l*(theta_r-k);
        bkmb.BKmb2();
        const auto search_end=std::chrono::steady_clock().now();
        tms search_time=search_end-search_begin;
        printf("Search finished! and cost time %lf \n",search_time.count());
        string find=(!bkmb.exist)?"not find":"has find";
        cout<<find<<endl;
        if(bkmb.exist)
        {
            if(maximum<bkmb.edges)
            {
                maximum=bkmb.edges;
                mbp=bkmb.maxbp;
                vector<xypair>::iterator it;
                for(it=XYPairs.begin();it!=XYPairs.end();)
                {
                    if(it->getupperbound()<=maximum)
                    {
                        // printf("%d,%d 's ub < current result, prunned \n",it->getx(),it->gety());
                        XYPairs.erase(it);
                    }
                    else
                    {
            ...
            if(low<bkmb.maxbp.right_size) low=bkmb.maxbp.right_size;
        }
        else
        {
    ...
    int ymin=ymax;
    for(findmax=XYPairs.begin();findmax!=XYPairs.end();findmax++)
    {
        if(ymin>findmax->gety())
        {
            ymin=findmax->gety();
        }
    }
    ...
    for(findmax=XYPairs.begin();findmax!=XYPairs.end();findmax++)
    {
        if(findmax->gety()!=ymin) continue;
        if(xmax<findmax->getx())
        {
            xmax=findmax->getx();
        }
    }
    ...
    while(true&&xmax>theta_l)
    {
        ...
        if(bkmb.exist)
        {
            ...
            if(low<bkmb.maxbp.left_size) low=bkmb.maxbp.left_size;
        }
        else
        {
            if((x!=theta_l)||(y!=theta_r))
    ...
    cout<<"now print the maximum k-biplex"<<endl;
    cout<<mbp;
	return 0;
}