...
class Core{
    public:
        ...
        void Coredecompose();
        int GetMaxcore();
        XYPair GetPairs(int left,int right);
        int GetMaxY(int x);
        int GetMaxX(int y);
        int *G_index;
    private:
...
XYPair Core::GetPairs(int left,int right){
    map<int,xypair> xp;
    map<int,xypair> yp;
    for(int x = left; x<max_v+K; x++){
        ...
        if(max_y < 2*K+1)
            break;
        xypair temp(right,max_y);
        xp.insert(map<int,xypair>::value_type(x,temp));
    }
    for(int y = right; y<max_u+K; y++){
        int max_x = GetMaxX(y);
        if(max_x < 2*K+1)
            break;
        xypair temp(right,max_x);
        yp.insert(map<int,xypair>::value_type(y,temp));
    }
    return XYPair(xp,yp);
}
